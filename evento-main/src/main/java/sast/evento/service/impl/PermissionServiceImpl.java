...
import sast.evento.config.AuthRegister;
...
public class PermissionServiceImpl implements PermissionService {
    ...
    public List<Action> getAllAdminPermissions() {
        return AuthRegister.actionName2action.values().stream()
                .filter(action -> action.getAuth().equals(Auth.ADMIN))
    ...
    public List<Action> getAllManagerPermissions() {
        return AuthRegister.actionName2action.values().stream()
                .filter(action -> action.getAuth().equals(Auth.MANAGER))
    ...
    public List<Action> getUserAdminPermissions(String userId) {
        return permissionServiceCacheAble.getPermission(userId, 0).getMethodNames().stream()
                ...
                .map(methodName -> AuthRegister.actionName2action.get(methodName))
                .toList();
    }
    ...
    public List<Action> getPublicActions() {
        return AuthRegister.actionName2action.values().stream()
                .filter(action -> action.getAuth().equals(Auth.PUBLIC))
                .toList();
    }
    ...
    public List<Action> getLoginAction() {
        return AuthRegister.actionName2action.values().stream()
                .filter(action -> action.getAuth().equals(Auth.LOGIN))
                .toList();
    }
